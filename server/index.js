...
//connects to database and creates a table if it doesn't exist. Gives a "Connected to the Database!!" prompt once connected.
...
app.get("/search/:search_item", async(req, res) => {
	...
	var url = `http://newsapi.org/v2/everything?q=${search}&searchIn=title&pageSize=100&sortBy=publishedAt&apiKey=${apiKey}`;
	console.log(url);
	news_get = await axios.get(url);
	//It sends a response to the client with a status code of 200 and a message "Not found" along 
	//with an empty array for the articles(if no articles found).
	...
	//Send a message of number of articles found.
	res.status(statusCode).send({
		articles: news_get.data.articles,
		message: `${news_get.data.articles.length} articles found.`
	})
	res.end();
...
//Makes a SELECT query to the MySQL database to retrieve the row of data associated.
